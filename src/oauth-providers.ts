...
function selectedProvider(
  ...
  const selected = selectedIds === undefined || selectedIds.length === 0
    ? undefined
    : new Set(selectedIds)
  return {
    ...base,
...
function stripApiKeyBeforeStream(provider: Provider<Api>): Provider<Api> {
  return {
    ...
    stream: (model, context, options) => provider.stream(
      model,
      context,
      options === undefined ? undefined : { ...options, apiKey: undefined },
    ),
...
export const CODEX_OAUTH_PROVIDER: OAuthProviderDefinition = {
  ...
  requestProvider: selectedIds => selectedProvider(
    createCodexProvider(),
    selectedIds,
    requestTokenAuth('OpenAI Codex OAuth token', false),
  ),
}
...
export const KIMI_CODE_OAUTH_PROVIDER: OAuthProviderDefinition = {
  ...
  requestProvider: selectedIds => stripApiKeyBeforeStream(selectedProvider(
    createKimiProvider(),
    selectedIds,
    requestTokenAuth('Kimi Code OAuth token', true),
  )),
}
