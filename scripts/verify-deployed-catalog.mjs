...
for (const [slug, route] of routes) {
  ...
  if (!authenticated && group !== undefined) failures.push(`${route}: unauthenticated but still advertises ${group.models.length} model(s)`)
  if (group !== undefined && !/\(OAuth\)$/u.test(group.name)) failures.push(`${route}: provider name lacks (OAuth): ${group.name}`)
  report.push(`${route}: ${authenticated ? 'authenticated' : 'unauthenticated'} → ${group === undefined ? 'hidden' : `${group.models.length} model(s), ${group.name}`}`)
}
...
if (agyExpectation === 'signed-out' && agy !== undefined) failures.push(`agy: expected signed-out but advertises ${agy.models.length} model(s)`)
if (agyExpectation === 'signed-in' && agy === undefined) failures.push('agy: expected signed-in but absent from model catalog')
if (agy !== undefined && agy.name !== 'Google Antigravity (OAuth)') failures.push(`agy: provider name lacks OAuth label: ${agy.name}`)
