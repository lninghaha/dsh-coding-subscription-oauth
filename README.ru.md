...
> Ещё не опубликовано: исправлено разрешение моделей OAuth в DSH `0.1.5-alpha.2` и `0.1.5-rc.1`. Проверенный BOM установки остаётся на `0.1.1-rc.2`; область проверок новых версий описана в [INSTALL.md](INSTALL.md).
...
> **Обновление:** Следуйте пошаговым инструкциям по версиям в [`INSTALL.md`](INSTALL.md). `0.6.5` сохраняет общий runtime диспетчера на `dsh-coding-oauth-core@0.1.2` и `undici@7.29.0` и ограничивает reveal/rotate ключа Gateway доступом loopback; миграция конфигурации, учётных данных, данных или маршрутов не требуется. Grok Imagine сохраняет явно закреплённый dispatcher. Релизы начиная с `0.6.2` включают строгое исправление запуска Cordis-инъекции и поддержку DSH `0.1.1-rc.2`; сохраните файлы profile/config/учётных данных и перезапустите существующий процесс DSH Web только один раз после обновления. При совместном использовании Hub и Subscription `dsh-coding-oauth-core@0.1.2` с `undici@7.29.0` — их общий runtime-pin, а не отдельный плагин DSH.
...
После установки перезапустите существующий процесс DSH Web. Проверка на реальном развёртывании:
...
Страница настроек разделена на четыре верхние вкладки: **Accounts**, **Gateway**, **Capabilities** и **About**. Карточки вошедших провайдеров сворачиваются в компактное резюме и разворачиваются для редактирования моделей. Предпросмотр CLI pull занимает всю ширину, а статус Imagine показан на вкладке Capabilities.
...
По умолчанию **выключен**. При включении запускается изолированный `node:http`-сервер (не веб-порт DSH) на `127.0.0.1:18080`, переиспользующий те же вошедшие OAuth-сессии:
