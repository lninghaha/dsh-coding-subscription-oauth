...
*/
function grokBuildReasoningMap(levels) {
	...
	return [
		{
			...
			reasoning: true,
			thinkingLevelMap: grokBuildReasoningMap([
				"low",
				"medium",
				"high"
			]),
...
function listingRows(body) {
	return Array.isArray(body) ? body : isRecord$1(body) && Array.isArray(body["data"]) ? body["data"] : isRecord$1(body) && Array.isArray(body["models"]) ? body["models"] : [];
}
function isPiThinkingLevel(value) {
	return PI_THINKING_LEVELS.includes(value);
}
/**
...
* as supported.
*/
function thinkingLevelMapFromLiveEfforts(efforts) {
	if (!Array.isArray(efforts)) return void 0;
	const offered = { off: null };
...
function parseLiveRow(row) {
	...
	if (!isRecord$1(row) || typeof row["id"] !== "string" || row["id"].length === 0) return void 0;
	const descriptor = { id: row["id"] };
	...
	if (typeof contextWindow === "number" && Number.isFinite(contextWindow) && contextWindow > 0) descriptor.contextWindow = contextWindow;
	if (row["supports_reasoning_effort"] === false) descriptor.reasoning = false;
	else {
		const thinkingLevelMap = thinkingLevelMapFromLiveEfforts(row["reasoning_efforts"]);
		if (thinkingLevelMap !== void 0) {
			descriptor.reasoning = true;
			descriptor.thinkingLevelMap = thinkingLevelMap;
		} else if (row["supports_reasoning_effort"] === true) descriptor.reasoning = true;
	}
	return descriptor;
}
...
function templateFor(id, catalog) {
	...
	const fallback = catalog.find((model) => model.id === "grok-4.6") ?? catalog[0];
	...
	if (/(^|[-_])4\.5($|[-_])/u.test(lower)) return catalog.find((model) => model.id === "grok-4.5") ?? fallback;
	return catalog.find((model) => model.id === "grok-4.6") ?? fallback;
}
function applyLiveOverlay(model, overlay) {
	if (overlay === void 0) return model;
	return {
		...model,
		id: overlay.id,
		...overlay.name === void 0 ? {} : { name: overlay.name },
		...overlay.contextWindow === void 0 ? {} : { contextWindow: overlay.contextWindow },
		...overlay.reasoning === void 0 ? {} : { reasoning: overlay.reasoning },
		...overlay.thinkingLevelMap === void 0 ? {} : { thinkingLevelMap: overlay.thinkingLevelMap }
	};
}
/** Turn a live id into a pi-ai model, inheriting baseline metadata when possible. */
function materializeLiveModel(id, catalog = catalogModels(), overlay) {
	const template = templateFor(id, catalog);
	return applyLiveOverlay(template.id === id ? template : {
		...template,
		id,
		name: titleCaseId(id)
	}, overlay);
...
*/
function mergeLiveCatalog(catalog, liveIds, liveModels = []) {
...
function preferredGrokBuildModelFrom(models) {
	if (new Set(models.map((model) => model.id)).has("grok-4.6")) return DEFAULT_GROK_BUILD_MODEL;
	return models[0]?.id ?? "grok-4.6";
}
/**
* Fetch the account-visible models from `/v1/models-v2` with the CLI
* fingerprint headers. Throws a secret-free error on failure.
*/
async function fetchLiveModels(accessToken, signal) {
	let response;
	try {
...
*/
const MIN_OAUTH_VALIDITY_MS = 6e4;
/**
...
function createGrokBuildAdapter(session, resolveAttachments) {
	return new PiAiAdapter({
		profiles: () => /* @__PURE__ */ new Map([[GROK_BUILD_ROUTE, profile(GROK_BUILD_ROUTE, "xAI Grok Build", session.provider(), void 0, grokBuildFingerprintHeaders())]]),
		resolveApiKey: async () => resolveOAuthToken("Grok Build", () => session.models.getAuth("xai", { minOAuthValidityMs: MIN_OAUTH_VALIDITY_MS })),
		resolveAttachments
	});
}
/** Create the four-route OAuth adapter while preserving each pi-ai native id. */
function createCodingOAuthAdapter(grok, subscriptions, resolveAttachments, retryPolicy) {
	...
	return new AliasLlmAdapter(new PiAiAdapter({
		profiles: () => {
			...
			profiles.set(GROK_BUILD_ROUTE, profile(GROK_BUILD_ROUTE, "xAI Grok Build", grok.provider(), retryPolicy, grokBuildFingerprintHeaders()));
			for (const session of subscriptions) profiles.set(session.definition.nativeProviderId, profile(session.definition.nativeProviderId, session.definition.displayName, session.provider(), retryPolicy));
			return profiles;
		},
		resolveApiKey: async (provider) => {
			if (provider === "grok-build") return resolveOAuthToken("Grok Build", () => grok.models.getAuth("xai", { minOAuthValidityMs: MIN_OAUTH_VALIDITY_MS }));
			const session = byNativeId.get(provider);
			if (session === void 0) throw new LlmError(`Unknown OAuth provider "${provider}"`, "NO_ADAPTER");
			return resolveOAuthToken(session.definition.displayName, () => session.models.getAuth(session.definition.nativeProviderId, { minOAuthValidityMs: MIN_OAUTH_VALIDITY_MS }));
		},
		resolveAttachments
	}), aliases, policies);
}
...
* bare upstream 401.
*/
async function resolveOAuthToken(displayName, getAuth) {
	let auth;
	try {
		auth = await getAuth();
	} catch (error) {
		throw new LlmError(`${displayName} could not refresh its sign-in (${error instanceof Error ? error.message : String(error)}). Open Settings → Coding OAuth and sign in again.`, "MISSING_CREDENTIAL", { cause: error });
	}
	const apiKey = auth?.auth.apiKey;
	if (apiKey === void 0 || apiKey.length === 0) return missingCredential(displayName);
	return apiKey;
}
//#endregion
...
var GrokBuildSession = class {
	...
	*/
	async invalidateAccessToken() {
		await this.store.invalidate("xai");
	}
	async logout() {
		try {
