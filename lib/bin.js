...
function listingRows(body) {
	return Array.isArray(body) ? body : isRecord(body) && Array.isArray(body["data"]) ? body["data"] : isRecord(body) && Array.isArray(body["models"]) ? body["models"] : [];
}
...
function parseLiveRow(row) {
	...
	if (!isRecord(row) || typeof row["id"] !== "string" || row["id"].length === 0) return void 0;
...
async function grokStatus() {
	...
	if (session.catalogError !== void 0) process.stderr.write(`dsh-coding-oauth: live models-v2 failed: ${session.catalogError}\n`);
	return true;
}
