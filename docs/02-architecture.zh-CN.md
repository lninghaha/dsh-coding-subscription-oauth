...
正式包名与仓库名是 **`dsh-coding-subscription-oauth`**。旧 GitHub 地址仍指向同一条 `main`，因此旧的 `dsh plugin add github:lninghaha/dsh-grok-build` 仍会安装更名后的包。第一次公开 npm / GitHub Release 是 **`0.4.1`**。当前版本是 **`0.6.5`**（`dsh plugin --profile web add dsh-coding-subscription-oauth@0.6.5`），精确验证 DSH **`0.1.1-rc.2`**。`0.1.5-rc.1` 仅作为未验证 BOM 候选记录；客户端 inject 不再要求 `@deepseek-ai/dsh-client-runtime`（该候选宿主上不存在此包）。GitHub 与本地 tarball 安装仍然有效。
