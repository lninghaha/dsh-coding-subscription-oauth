...
源码按域分目录（`src/oauth/`、`src/gateway/`、`src/capability/`、`src/codex/`、`src/core/`、`src/client/`）。包入口仍在 `src/index.ts`、`src/bin.ts`、`src/invariant.ts`。

- `oauth/store.ts`：一个文件只拥有一个 provider credential；保留旧 Grok store API；`invalidate()` 在上游 AUTH 拒绝后把 `expires` 回写到过去。
- `oauth/oauth-providers.ts`：Codex/Kimi/Claude 定义、route metadata、请求 token bridge。
- `oauth/oauth-session.ts`：登录、刷新、静态模型目录和模型选择缓存。
- `oauth/oauth-sources.ts`：白名单官方 Grok/Codex/Kimi/Claude CLI 发现；加固的 lstat/`O_NOFOLLOW`/属主/权限/普通文件/大小读取；一次性预览票据（五分钟、最多 32 张）；从不写入官方 CLI 文件。
- `oauth/oauth-import-routes.ts`：同源拉取 HTTP API（发现 → 预览 → 提交/取消），写入发生在目标 store 锁内。
- `core/alias-adapter.ts`：转换 Harness route、不修改 pi-ai model.provider，并在 `listModels()` 前执行 credential gate；未认证或凭据读取失败返回空目录，provider group 名使用 `(OAuth)`。AUTH finish 时作废本地令牌，让 harness 重试先刷新。
- `core/adapter.ts`：组合 Grok 与三个 subscription profile；向 pi-ai 要求至少 60 秒剩余有效期，并注册包含 AUTH 与瞬时故障码的 retryPolicy。可选包装 `codex-oauth-fast`，显示为 **已请求 Fast**。
- `oauth/auth-routes.ts`：旧 Grok API + 新统一 `/plugins/dsh-grok-build/oauth/*`；JSON 写请求使用 64 KiB 有界读取器，无效/超限 body 分别返回 400/413。
- `capability/capability-settings.ts`：默认关闭、立即生效的开关与限制（搜索 1–20、图像 1–4、产物 TTL 1 小时–7 天）。
- `capability/capability-routes.ts`：无密钥的能力快照，以及可选的 Codex 用量和 Imagine 凭据状态路由。
- `capability/capability-runtime.ts`：按 live 开关绑定/解绑搜索、工具，以及仅在最新 priority catalog 后发布 Fast 路由。
- `capability/capability-tools.ts`：可选 Codex / Grok Imagine 工具定义；执行时重新读取开关。
- `codex/codex-http.ts`：需打开的私有 `chatgpt.com/backend-api` 客户端（仅 HTTPS、仅第一方主机）。
- `codex/codex-search.ts` / `codex/codex-usage.ts` / `codex/codex-images.ts`：可选搜索、配额，以及固定 `gpt-image-2` 生成/编辑（编辑要求当前会话顶层附件所有权）。
- `codex/codex-model-capabilities.ts`：live Codex service-tier 缓存；Fast 资格失败关闭；注入 `service_tier: priority` 与路由提示。
- `capability/grok-imagine.ts`：官方 `api.x.ai` Imagine 客户端（`grok-imagine-image-2.0` / `grok-imagine-video-1.5`）；`XAI_API_KEY` 只通过 DSH 凭据；MIME/大小/超时/重定向/DNS 下载控制；冻结主机 `imgen.x.ai`、`videogen.x.ai`、`vidgen.x.ai`。
- `capability/imagine-routes.ts`：生成图像与视频产物的同源 loopback GET 路由。
- `capability/media-store.ts`：属主私有产物库（单件与唯一对象总量均硬限 256 MiB，最长七天）。
- `client/`：四个原生账号卡片、CLI 拉取、能力开关、网关控制，以及外部 Antigravity 状态卡片（`locales.ts` 提供 `en` / `zh`）。
- `core/proxy.ts`：process-wide undici dispatcher，但只代理审核过的域名白名单。
- `gateway/gateway*.ts`：可选的隔离 loopback OpenAI/Anthropic 兼容 HTTP 服务（默认关；独立于 DSH web 端口）。
...
正式包名与仓库名是 **`dsh-coding-subscription-oauth`**。旧 GitHub 地址仍指向同一条 `main`，因此旧的 `dsh plugin add github:lninghaha/dsh-grok-build` 仍会安装更名后的包。第一次公开 npm / GitHub Release 是 **`0.4.1`**。安装最新发布版：`dsh plugin --profile web add dsh-coding-subscription-oauth` 或 `npm install dsh-coding-subscription-oauth`（详见 `INSTALL.zh-CN.md`）。GitHub 与本地 tarball 安装仍然有效。
