...
本仓库原名 **`dsh-grok-build`**。普通用户请安装已发布的 npm 包（**最新版**）：

```bash
# 推荐：从 npm 安装最新版到 web profile
dsh plugin --profile web add dsh-coding-subscription-oauth

# 或使用 npm 直接安装最新版，再挂到 DSH profile
npm install dsh-coding-subscription-oauth
```

上述命令安装 registry 上的**最新**发布版。只有需要钉死某个 tag 时才加版本号——见 [CHANGELOG.md](CHANGELOG.md) / npm 历史版本。当前文档对应的发布线是 **0.5.x**。

CLI 主命令是 `dsh-coding-oauth`（旧命令 `dsh-grok-build` 仍可用）。为兼容已有 profile，Cordis id 仍是 `llm-grok-build-oauth`，设置页 HTTP 路径仍是 `/plugins/dsh-grok-build/*`，凭据文件名不变。
...
- Node.js 22.19+
- 需要使用的个人编码订阅；没有 Claude/Google 账号也可以先安装其他路由
- 部分网络需要 HTTP/HTTPS 代理

## 安装

```bash
# 推荐：通过 DSH 插件管理器安装最新 npm 发布版
dsh plugin --profile web add dsh-coding-subscription-oauth
...
# Google Antigravity 可选依赖（固定审核过的版本）
...
或在 **设置 → 编码 OAuth → Gateway** 打开。Bearer key 存在 `$DSH_HOME/.coding-oauth-gateway.json`。不要绑定 `0.0.0.0`。
...
仅在当前网络确实需要代理时设置 `NODE_USE_ENV_PROXY` / `HTTPS_PROXY`。不要把 Google credential export 粘贴到聊天或日志。
...
默认进入代理的域名组：xAI/Grok Build、OpenAI Codex、Claude/Anthropic、Google OAuth/Cloud Code。Kimi Code 中国流量默认直连；只有 `proxyKimi: true` 才代理。
...
七项订阅能力开关默认全部关闭，打开后立即生效（无需重启）：`codexSearch`、`codexImages`、`codexImageEdits`、`codexUsage`、`codexFast`、`grokImagineImage`、`grokImagineVideo`。

数值控制为 `searchResults`（1–20，默认 5）、`imageCount`（1–4，默认 1）、`videoArtifactTtlMs`（1 小时–7 天，默认 7 天；界面以 1–168 小时显示）。降低视频保留时间会立即缩短并清理已有产物；提高只影响之后生成的产物。管理员可在插件配置的 `capabilities` 下提供不含秘密的 composition 默认值；用户 live 设置会覆盖该 base。

`codex-oauth-fast` 仅在最新一次 live catalog 标明至少有一个 `priority` 可用模型后才会出现。请求发送 `service_tier: priority` 和路由提示；界面写 **已请求 Fast**，不保证延迟。Codex 搜索/用量/图像是需打开的私有 `chatgpt.com/backend-api` 端点；图像固定 `gpt-image-2`；编辑只接受当前会话顶层、本会话持有的附件。

Grok Imagine 只走官方 `https://api.x.ai`（`grok-imagine-image-2.0` / `grok-imagine-video-1.5`），凭据是独立的 DSH 引用 `XAI_API_KEY`——不用 Grok OAuth，也不回退进程环境变量。下载受 MIME / 大小 / 超时 / 重定向 / DNS 控制，冻结主机为 `imgen.x.ai`、`videogen.x.ai`、`vidgen.x.ai`；私有产物库单件与唯一对象总量均硬限 256 MiB、最长七天；只通过同源 loopback 路由提供。
...
- `grok-build/<model>`
- `codex-oauth/<model>`
- `codex-oauth-fast/<model>`（可选；仅在最新 live catalog 标明 `priority` 可用后出现）
- `kimi-code-oauth/<model>`
- `claude-code-oauth/<model>`
- `agy/<model>`（安装 dsh-agy 后）

这些别名避免与已有的 `xai`、`openai`、`kimi-coding` API-key routes 冲突。未认证的 OAuth route 不向模型选择器返回任何模型；认证后供应商名显示为 `(OAuth)`。

## Kimi 中国说明

Kimi Code 订阅 OAuth 使用 `https://auth.kimi.com`；推理使用 `https://api.kimi.com/coding`。`https://api.moonshot.cn/v1` 是按量 **Moonshot 开放平台** API-key 通道——没有可切换的「中国 OAuth 端点」。本插件使用独立的 `kimi-code-oauth` 路由，不影响已有的 `kimi-coding` API-key 配置。
...
均为 `0600`、原子写、文件锁保护。模型缓存为对应的 `*-models.json`，不含 token。Grok Imagine 使用 DSH 凭据引用 `XAI_API_KEY`。视频产物存入 `$DSH_HOME/.dsh-coding-subscription-oauth-media/`（目录 `0700`、文件 `0600`）。**任何 HTTP 状态、日志或 UI 都不得返回 token。**
...
以下命令面向维护者，需要在源码 checkout 中运行；通过 npm 安装的用户无需执行。另见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
`verify:deployed` 验证真实模型目录的认证门禁和 `(OAuth)` 标签。`smoke:deployed` 会执行 Codex/Kimi 的 tool-call 与第二个用户 turn（覆盖 `INVALID_REPLAY_STATE`），随后恢复显式指定的默认模型并归档测试会话；未提供 `DSH_RESTORE_*` 时脚本拒绝运行。
