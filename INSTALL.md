...
- 在同一个 **web profile** 中先保证 `dsh-coding-oauth-core@0.1.2` 可从 npm 解析，再安装 Subscription `0.8.3`（以及需要的 `dsh-hub-oauth-gateway@1.13.3`）。Core 只是共享 npm 依赖，不是单独的 DSH 插件，用户不需要执行 `dsh plugin add dsh-coding-oauth-core`。
- 共装 Hub 与 Subscription 时，先安装 `dsh-hub-oauth-gateway@1.13.3` 与 Subscription `0.8.3`，完成后只重启一次现有 DSH Web 进程；Hub 提供完整用量中心，Subscription 显示紧凑状态入口。只升级 Subscription 仍可独立工作。
