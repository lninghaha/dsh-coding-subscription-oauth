...
import { KIMI_CODE_OAUTH_PROVIDER } from "../../src/oauth/oauth-providers.ts";
import { OAuthProviderSession } from "../../src/oauth/oauth-session.ts";
import { OAuthCredentialFileStore } from "../../src/oauth/store.ts";
...
describe("OAuthProviderSession", () => {
	...
	it("invalidateAccessToken backdates the stored credential expiry", async () => {
		...
		const session = new OAuthProviderSession(KIMI_CODE_OAUTH_PROVIDER, undefined, store, join(dir, "models.json"));
