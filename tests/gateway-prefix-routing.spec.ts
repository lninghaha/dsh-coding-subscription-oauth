...
it("still accepts legacy opencode-go/ gateway prefixes during migration", async () => {
	...
	expect(JSON.parse(String((outbound.mock.calls[0] as unknown as [string, RequestInit])[1].body)).model).toBe(
		"test-chat",
	);
});
